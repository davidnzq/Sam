...
/*!*****************************!*\
  ...
  \*****************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _test_test_css__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../test/test.css */ "./src/test/test.css");
/**
...
class SimplePerformanceMonitor {
    ...
    recordResponse(startTime, success, responseTime) {
        if (success) {
...
class OptimizedApiClient {
    ...
    async optimizeText(text, isStrictMode, apiSettings) {
        // 初始化返回值
        let result = {
            success: false,
            error: "未知错误"
        ...
        if (cachedResponse) {
            ...
            // 记录缓存命中
            this.performanceMonitor.recordResponse(requestStartTime, true, Date.now() - requestStartTime);
        ...
        try {
            ...
            try {
                ...
                // 记录响应完成
                this.performanceMonitor.recordResponse(requestStartTime, true, Date.now() - startCallTime);
                return apiResponse;
            }
            catch (apiError) {
                // 清除超时
            ...
            // 记录错误响应
            this.performanceMonitor.recordResponse(requestStartTime, false, Date.now() - requestStartTime);
    ...
    generateCacheKey(text, isStrictMode) {
        return `${isStrictMode ? 'strict:' : 'basic:'}${text}`;
    }
    findSimilarTextCacheKey(text, isStrictMode) {
     ...
     */
    handleFallbackProcessing(text, isStrictMode) {
...
let testTextArea;
let testModeSelect;
let useCacheModeCheckbox;
let showPerformanceStatsCheckbox;
let optimizeBtn;
let clearBtn;
let clearCacheBtn;
let loadingIndicator;
let testResults;
let originalTextElement;
let optimizedTextElement;
...
async function initialize() {
    // 获取页面元素
    testTextArea = document.getElementById('testText');
    testModeSelect = document.getElementById('testMode');
    useCacheModeCheckbox = document.getElementById('useCacheMode');
    showPerformanceStatsCheckbox = document.getElementById('showPerformanceStats');
    optimizeBtn = document.getElementById('optimizeBtn');
    clearBtn = document.getElementById('clearBtn');
    clearCacheBtn = document.getElementById('clearCacheBtn');
    loadingIndicator = document.getElementById('loadingIndicator');
    testResults = document.getElementById('testResults');
    originalTextElement = document.getElementById('originalText');
    optimizedTextElement = document.getElementById('optimizedText');
...
async function optimizeText() {
    ...
    try {
        ...
        // 调用API优化文本
        const result = await apiClient.optimizeText(textToOptimize, isStrictMode, {
...
// 显示优化结果
function displayResults(originalText, result, processingTime) {
    // 设置处理时间
...
// 添加统计项
function addStatItem(label, value, valueClass = '') {
...
// 添加指标项
function addMetricItem(label, value) {
