...
var ErrorType;
(function (ErrorType) {
    ErrorType["API_NOT_CONFIGURED"] = "API_NOT_CONFIGURED";
    ...
    ErrorType["SCRIPT_INJECTION_ERROR"] = "SCRIPT_INJECTION_ERROR";
    ErrorType["MESSAGE_ERROR"] = "MESSAGE_ERROR";
...
class ErrorHandler {
    ...
    static getErrorInfo(error) {
        ...
        if (error === ErrorType.CONNECTION_ERROR ||
            (typeof error === 'string' && (error.includes('connection') ||
    ...
    // 处理连接错误
    static handleConnectionError(error, retryCallback) {
        const errorInfo = this.getErrorInfo(error);
        this.logError(error);
    ...
    // 重试函数
    static retry(fn, maxRetries = 3, delay = 500) {
...
/*!*********************************!*\
  ...
  \*********************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _utils_error_handler__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../utils/error-handler */ "./src/utils/error-handler.ts");
/**
 * LongPort AI 助手 - 后台服务
 */
...
// 处理文本优化的通用函数
async function handleOptimizeText(tabId, selectedText) {
        ...
        try {
            ...
            if (sendError instanceof Error && sendError.message === '发送消息超时') {
                ...
                try {
                    ...
                    const resendPromise = new Promise((resendResolve, resendReject) => {
                        chrome.tabs.sendMessage(tabId, {
                            type: 'CONTEXT_MENU_OPTIMIZE',
                            text: selectedText
                        }, (response) => {
...
// 模拟API响应（用于开发和测试）
async function mockApiResponse(text) {
    // 模拟处理延迟
    await new Promise(resolve => setTimeout(resolve, 1500));
    // 简单的文本优化逻辑
...
// 调用API优化文本
async function optimizeTextWithApi(text, isStrictMode, apiSettings) {
    try {
        // 检查API配置
        ...
        const systemPrompt = isStrictMode
            ? `你是「长桥规范文案统一优化」的严格的文案优化助手。

在不改变原意、事实、数字与承诺的前提下，按长桥规范统一修正错别字、语法、大小写、标点、空格与排版，输出可直接发布的文本。文风统一为：专业、清晰、友好。

请严格按照一下要求优化文本：

1. 最小改动：不改事实/时间/数字；不新增或删减功能点；不引入外部信息；避免营销腔与夸张语。  
2. 语气与称谓：专业克制且友好；尽量不用感叹号（禁"！！"）；默认称呼"你"，仅在特定个体且正式场景用"您"。  
3. 品牌与命名：Longbridge（合成词，非 Long Bridge；URL 用小写 longbridge，如 longbridge.com）。  
4. 专有名大小写：按官方写法（Android、iOS、iPhone、Google、Apple、GitHub、IPO、ETF…）；正式文案优先用英文品牌名。  
5. 中英空格：英文与中文（非标点）之间留 1 个半角空格。  
6. 数字与单位：数字与中文之间留空格；数字与 %、°C、2x/3n 紧连；其他单位与数字间空格（10 km、5 ms）；货币示例 500 USD、1,000 HKD；中文语境可优先中文数词（如"五个"）。  
7. 标点：中/英混排句内用中文全角标点；整句英文用半角标点；中文标点旁不加多余空格；避免连续两枚中文标点。  
8. 括号与引号：默认半角 ()（UI/数字/代码/证券），如 阿里巴巴 (BABA.US)、当前在线 (50 人)；括号内首尾不留空格；全中文语境可用（）；避免【】，推荐 [] 或「」。  
9. 段落与列表：段落间空一行、段首不留空白；富文本优先用列表；纯文本有序用"1. "（数字后空格），无序用"- "（短横+空格）。  


【处理顺序】
理解内容与对象 → 语法/错别字 → 专名/大小写 → 中英与数字空格 → 标点/括号/引号 → 段落/列表 → 精炼（强化专业、清晰、友好）→ 最小改动复核。
优化原则：保持与原文语义一致，提升表达清晰度和专业性。
【输出规范（强制）】
仅返回优化后的成品文本；不返回任何解释、标题、JSON、差异说明、提示语、引号、代码块或前后缀标注。保持原有结构类型（段落/标题/列表），必要时为合规进行轻度重排。`
            : "你是一个专业的文案优化助手。请按照以下要求优化文本：1. 语法校正：修正错误的语法结构 2. 标点规范：统一标点符号使用 3. 中英混排规则：优化中英文混合排版 4. 语言风格：采用专业、清晰、友好的表达方式 5. 语调控制：保持克制，避免过度营销和夸张表达。优化原则：保持与原文语义一致，提升表达清晰度和专业性。";
        // 准备OpenAI标准的请求数据
        const openaiRequest = {
        ...
        console.log('当前模式:', isStrictMode ? '严格模式' : '普通模式');
        ...
        const response = await _utils_error_handler__WEBPACK_IMPORTED_MODULE_0__["default"].retry(makeApiCall, 3, 500);
        // 检查响应状态
        if (!response.ok) {
            try {
                // 尝试解析错误响应为JSON
        ...
        try {
            ...
            // JSON解析错误
            _utils_error_handler__WEBPACK_IMPORTED_MODULE_0__["default"].logError(jsonError);
            return {
                success: false,
                error: 'JSON解析错误',
        ...
        _utils_error_handler__WEBPACK_IMPORTED_MODULE_0__["default"].logError(error);
        const errorInfo = _utils_error_handler__WEBPACK_IMPORTED_MODULE_0__["default"].getErrorInfo(error);
        return {
            success: false,
        ...
        chrome.storage.sync.get(['apiEndpoint', 'apiKey', 'strictMode'], (items) => {
            ...
            const processOptimizeRequest = async () => {
                try {
                    ...
                    try {
                        // 调用API优化文本
                        const result = await optimizeTextWithApi(message.text, message.isStrictMode !== undefined ? message.isStrictMode : settings.strictMode, settings);
                        return result;
                    }
                    catch (apiError) {
