// tests/mock/lint.js
// lint模块的CommonJS版本，用于测试

/**
 * 对文本进行本地校验，检查是否符合样式指南
 * @param {string} text 要校验的文本
 * @param {object} scene 场景指南对象
 * @returns {Array} 命中的规则列表
 */
function lintAgainstGuide(text, scene) {
  const hits = [];

  // 禁用词（阻断）
  if (scene.banned_terms) {
...
module.exports = { lintAgainstGuide };