...
// 金融术语词典（可扩展）
const FIN_TERMS: Record<string, { explain: string; analogy: string }> = {
  "市盈率": {
...
// 用户自定义词条存储
let customTerms: Record<string, { explain: string; analogy: string; createdAt: number }> = {};

// 已掌握的词条存储
let masteredTerms: Set<string> = new Set();
...
// 标记词条为已掌握
async function markTermAsMastered(term: string) {
...
// 取消词条的已掌握状态
async function unmarkTermAsMastered(term: string) {
...
// 添加自定义词条
async function addCustomTerm(term: string, explain: string, analogy: string) {
  customTerms[term] = {
    explain,
    analogy,
    createdAt: Date.now()
...
async function generateTermExplanation(term: string): Promise<{ explain: string; analogy: string } | null> {
  try {
    // 检查扩展是否可用
    ...
    const resp = await new Promise<{ ok: boolean; data?: { explain: string; analogy: string }; error?: string }>((resolve, reject) => {
      try {
...
// 备用方案：生成基础解读和类比
function generateFallbackExplanation(term: string): { explain: string; analogy: string } {
  // 检测术语语言
...
// 获取所有词条（内置 + 自定义）
function getAllTerms(): Record<string, { explain: string; analogy: string }> {
...
// 显示右键菜单
function showContextMenu(x: number, y: number, selectedText: string) {
...
// 显示自定义词条弹窗
function showCustomTermTooltip(term: string, position: { x: number, y: number }) {
...
// 显示添加词条弹窗
function showAddTermTooltip(term: string, position: { x: number, y: number }) {
  ...
  const explainInput = tooltipContentEl.querySelector('.explain-input') as HTMLTextAreaElement;
  const analogyInput = tooltipContentEl.querySelector('.analogy-input') as HTMLTextAreaElement;
  
  // AI自动生成按钮事件
  if (autoGenerateButton) {
    autoGenerateButton.addEventListener('click', async () => {
      (autoGenerateButton as HTMLButtonElement).textContent = '🔄 生成中...';
      (autoGenerateButton as HTMLButtonElement).disabled = true;
      ...
      try {
        ...
        (autoGenerateButton as HTMLButtonElement).textContent = '🤖 AI自动生成';
        (autoGenerateButton as HTMLButtonElement).disabled = false;
      }
    });
  }
...
// 显示新增词条的解读弹窗（AI生成内容）
async function showNewTermTooltip(term: string, position: { x: number, y: number }) {
  const host = getTooltip();
  if (!tooltipContentEl) return;
...
function ensureStyles() {
  if (document.getElementById('yiyuan-style')) return;
...
function getEventElement(e: Event): Element | null {
  ...
  const path = typeof (e as any).composedPath === 'function' ? (e as any).composedPath() as any[] : [];
  const elFromPath = path.find(n => n instanceof Element) as Element | undefined;

  if (elFromPath) return elFromPath;

  const t = e.target as Node | null;
  if (t instanceof Element) return t;
  if (t && t.nodeType === Node.TEXT_NODE) return (t as Text).parentElement;
  return null;
}

// Shadow DOM tooltip
let tooltipHost: HTMLDivElement | null = null;
let tooltipContentEl: HTMLDivElement | null = null;
...
function showTooltip(term: string, anchorRect: DOMRect) {
  const allTerms = getAllTerms();
...
function createSidebar() {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (sidebar && !sidebar.contains(target as Node)) {
...
function detectFinancialTerms(text: string): string[] {
  const allTerms = getAllTerms();
...
function highlightTerms(root: Node, terms: string[]): number {
  if (terms.length === 0) return 0;
  ensureStyles();
  
  // 过滤掉已掌握的词条
  const termsToHighlight = terms.filter(term => !masteredTerms.has(term));
  if (termsToHighlight.length === 0) return 0;
  ...
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
    acceptNode(node) {
      ...
      if (BLOCKED_TAGS.has(parent.tagName)) return NodeFilter.FILTER_REJECT;
      if (parent.classList.contains('yiyuan-highlight')) return NodeFilter.FILTER_REJECT;
      
      // 安全地检查 parent 是否支持 closest 方法
      if (parent && typeof parent.closest === 'function') {
        if (parent.closest('.yiyuan-tooltip,[data-yiyuan-skip="1"]')) return NodeFilter.FILTER_REJECT;
      }
...
function scanAndHighlight(): number {
...
let scanDebounceId: number | null = null;

function triggerScanDebounced(delayMs = 400) {
  if (!featureEnabled) return;
  if (scanDebounceId !== null) window.clearTimeout(scanDebounceId);
